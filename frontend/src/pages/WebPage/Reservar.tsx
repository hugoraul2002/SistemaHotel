...
import { createCliente, createRegistroPago, createReservacion, registrarEnlacePago } from "../../services/PasarelaPagosService";
...
export default function Reserva() {
                ...
                if (response) {
                    ...
                    if (response.habitacionesDisponibles.length > 0) {
                        ...
                        if (response.otrasHabitacionesDisponibles.length > 0) {
                            mostrarToast('Existen habitaciones disponibles con un número inferior de capacidad del solicitado.', 'info');
                        } else {
    ...
    const footer = (
        <div className="flex justify-end">
            <Button label="Next" icon="pi pi-arrow-right" className="bg-black border-black" onClick={() => {
                if (dates === null || dates === undefined) {
                    mostrarToast('Ingrese las fechas para su reservación.', 'warn');
                    return;
                }
                if (dates[0] === null || dates[1] === null) {
                    mostrarToast('Complete el rango de fechas para su reservación.', 'warn');
                    return;
                }
    ...
    const handleRegistrarReserva = async () => {
        try {
            if (!selectedHabitacion || !cliente || dates?.length !== 2 || !validaDatosHuesped()) {
                mostrarToast('Debes seleccionar una habitación y completar tus datos.', 'warn');
                return;
            }
            ...
            if (clienteResponse) {
                ...
                if (reservacionResponse) {
                    ...
                    if (pago) {
                        console.log(pago);
                        const enlacePago = await registrarEnlacePago(selectedHabitacion, cliente, pago.idHash);
    ...
    const totalPagar =(dates!==null && dates!==undefined && dates[0]!== null && dates[1]!== null && selectedHabitacion) ?  selectedHabitacion.precio * dayjs(dates[1]).diff(dates[0], 'days') : 0;
    return (
        <div className="flex justify-center items-center h-screen w-screen px-4 py-4">
            <Toast ref={toast} />
            <Stepper ref={stepperRef} linear className="flex-auto" style={{ width: '100%', height: '100%' }}>
                    ...
                    <div className="w-full p-6 flex flex-col gap-4">
                        ...
                        <div className="w-full flex flex-col gap-8">
                            <Panel header={panel1Header}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
                                    {habitaciones.length > 0 && habitaciones.map((habitacion) => (
                                        <Card key={habitacion.id} className="shadow-lg max-w-[435px] w-full mx-auto">
                                            ...
                                            <div className="p-4">
                                                ...
                                                <Button label="Seleccionar" className="mt-4 w-full bg-gray-200 text-black border-gray-700" onClick={() => { setSelectedHabitacion(habitacion); stepperRef.current.nextCallback() }} />
                                            </div>
                                        </Card>
                                    ))}
                                </div>
                            ...
                            <Panel header={panel2Header} toggleable>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
                                    {otrasHabitaciones.length > 0 && otrasHabitaciones.map((habitacion) => (
                                        <Card key={habitacion.id} className="shadow-lg max-w-[435px] w-full mx-auto">
                                            <img src="https://imgs.search.brave.com/1v19-lKHrcxrgPF8qoRgm7zMT-av42YD8rwvI3wcPGM/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9tZWRp/YS5nZXR0eWltYWdl/cy5jb20vaWQvMTI5/OTA5ODM4NC9lcy9m/b3RvL2ludGVyaW9y/LW1vZGVybm8tZGVs/LWRvcm1pdG9yaW8t/Zm90by1kZS1zdG9j/ay5qcGc_cz02MTJ4/NjEyJnc9MCZrPTIw/JmM9XzJUUDNEeVRN/Y2pkTXRoTXFESm0y/Z2ppdWNjRnFyVkxu/VEtuSG91N3l2dz0"
                                                alt={habitacion.nombre}
                                                className="w-full h-40 object-cover rounded-t-lg" />
                ...
                <StepperPanel header="Resumen">
                            ...
                            {selectedHabitacion ? (
                                <div className="flex justify-between">
                                    <div>
                                        ...
                                        <p>Capacidad: {selectedHabitacion.numeroPersonas} personas</p>
                                        <hr className="my-2 border-gray-300"/>
                                        <p>Total a cancelar: Q. {totalPagar}</p>
                                    </div>
                                    <div>
                    ...
                    <div className="flex pt-4 justify-between">
                        <Button label="Regresar" icon="pi pi-arrow-left" className="bg-black border-black" onClick={() => stepperRef.current.prevCallback()} />
                        <Button label="PAGAR" icon="pi pi-credit-card" className="bg-black border-black"
